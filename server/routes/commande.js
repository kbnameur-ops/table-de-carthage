...
function chargerCarte() {
  ...
  const plats = db.prepare(`SELECT * FROM plats WHERE visible = 1 AND a_emporter = 1 ORDER BY position`).all();
  return categories
...
function validerPanier(body) {
  const plats = db.prepare(`SELECT * FROM plats WHERE visible = 1 AND a_emporter = 1`).all();
  const lignes = [];
...
commandeRouter.get('/commander', (req, res) => {
  res.render('commande', {
    categories: chargerCarte(), euros, erreurGenerale: null, erreurs: {}, valeurs: {},
    aujourdHui: aujourdHui(), session: req.session, csrfToken: res.locals.csrfToken,
  });
});

commandeRouter.post('/commander', verifierCsrf, (req, res) => {
  const b = req.body;
  const erreurs = {};
  ...
  const inserer = db.transaction(() => {
    const { lastInsertRowid: commandeId } = db.prepare(
      `INSERT INTO commandes (reference, client_id, date, heure, total_cents, message)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(reference, client.id, b.date, b.heure, panier.total, (b.message || '').trim());
    const insLigne = db.prepare(
      `INSERT INTO commande_lignes (commande_id, plat_id, nom, prix_cents, quantite) VALUES (?, ?, ?, ?, ?)`
    );
    for (const l of panier.lignes) insLigne.run(commandeId, l.plat_id, l.nom, l.prix_cents, l.quantite);
  });
  inserer();
...
commandeRouter.get('/commander/confirmation', (req, res) => {
  const commande = db.prepare(`SELECT * FROM commandes WHERE reference = ?`).get(req.query.ref || '');
  if (!commande) return res.redirect('/commander');
  const lignes = db.prepare(`SELECT * FROM commande_lignes WHERE commande_id = ?`).all(commande.id);
