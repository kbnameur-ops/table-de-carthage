...
async function occupation(serviceId, date, heure) {
  return une(
    `SELECT COALESCE(SUM(couverts), 0)::int AS couverts, COUNT(*)::int AS tables
     FROM reservations
     WHERE service_id = $1 AND date = $2 AND heure = $3 AND statut NOT IN ('annulee', 'absente')`,
...
export async function creneauxDisponibles(date, couverts) {
  ...
  const creneaux = await Promise.all(creneauxDuService(service).map(async heure => {
    const occ = await occupation(service.id, date, heure);
...
export async function creneauEncoreValide(date, heure, couverts) {
  const { service, creneaux } = await creneauxDisponibles(date, couverts);
